...
class ASTP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.global_context_att:
            context_mean = torch.mean(x, dim=-1, keepdim=True).expand_as(x)  # @ljj: 时间维度上的均值
            context_std = torch.sqrt(
                torch.var(x, dim=-1, keepdim=True) + 1e-7).expand_as(x)  #ljj: 时间维度上方差平方根
        ...
        alpha = torch.softmax(self.linear2(alpha), dim=2) # t维度上归一化
        mean = torch.sum(alpha * x, dim=2)  # @ljj: 时间维度上加权求和
